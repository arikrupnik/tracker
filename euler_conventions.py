...
class AzEl():
    """Euler angles as Azimuth/Elevation, intrinsic rotations with respect
...
def resultant_azel(base_rpy, turret_pt):
    """Computes resulting Azimuth/Alevation of camera from base attitude
       and Pan/Tilt of turret."""
    return AzEl(r=(base_rpy.r * turret_pt.r))

def test_resultant_azel():
    # simple addition on one axis
    att = RPY(0, 30, 0)
    pt = PT(0, 20)
    assert str(resultant_azel(att, pt)) == "a:   0.0 e: 50.0"

    # rotating the base rotates the turret
    att = RPY(0, 0, 90)
    assert str(resultant_azel(att, pt)) == "a:  90.0 e: 20.0"

    # yawing the base right and pitching it up adds turret tilt to base pitch
    att = RPY(0, 30, 90)
    assert str(resultant_azel(att, pt)) == "a:  90.0 e: 50.0"

    # yawing the base right and rolling it left decreases azimuth and elevation
    att = RPY(-30, 0, 90)
    assert str(resultant_azel(att, pt)) == "a:  79.7 e: 17.2"


def necessary_pt(base_rpy, target_azel):
    """Computes necessary Pan/Tilt for turret from base attitude to point
       at target Azimuth/Elevation."""
    return PT(r=(base_rpy.r.inv() * target_azel.r))

def test_necessary_pt():
    # simple subtraction on one axis
    att = RPY(0, 30, 0)
    azel = AzEl(0, 20)
    assert str(necessary_pt(att, azel)) == "p:   0.0 t:-10.0"

    # yawing the base requires unyawing the turret but has no effect on tilt
    att = RPY(0, 0, 90)
    assert str(necessary_pt(att, azel)) == "p: -90.0 t: 20.0"

    # yawing the base right and rollig it right subtracts base roll from target
    # elevation
    att = RPY(30, 0, 90)
    assert str(necessary_pt(att, azel)) == "p: -90.0 t:-10.0"
    ...
    # for azimuth
    att = RPY(0, 30, 90)
    assert str(necessary_pt(att, azel)) == "p: -79.7 t: 17.2"
